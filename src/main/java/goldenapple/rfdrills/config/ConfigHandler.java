  ...
  /*public static String energyUnitName;
    ...
    private void loadConfig(){
      /*energyUnitName = config.getString("energyUnit", Configuration.CATEGORY_GENERAL, "RF", "Energy unit to display in tooltips. Valid units: EU, RF, gJ (GalactiCraft Joules), J (Mekanism Joules)", new String[]{"RF", "EU", "gJ", "J"});
        ...
        shearsDefault = config.getBoolean("Shears Default", Configuration.CATEGORY_GENERAL, true, "Set this to false to disable shears functionality for lower tier chainsaws", "config.shearsDefault");
        ...
        int soulCrusherMiningLevel = config.getInt("Mining Level", "soulcrusher", 5, 0, Integer.MAX_VALUE, "Mining level of the tool (1 - can mine iron, 2 - can mine diamonds etc.)", "config.miningLevel");
        float soulCrusherEfficiency = config.getFloat("Efficiency", "soulcrusher", 12.0F, 0, Float.MAX_VALUE, "Mining speed of the tool", "config.efficiency");
        float soulCrusherDamage = config.getFloat("Damage", "soulcrusher", 8.0F, 0, Float.MAX_VALUE, "Damage that the tool does when attacking enemies", "config.damage");
        int soulCrusherEnchant = config.getInt("Enchantability", "soulcrusher", 20, 0, Integer.MAX_VALUE, "Enchantablity of the tool (0 - not enchantable)", "config.enchant");
        boolean soulCrusherCanBreak = config.getBoolean("Can Break", "soulcrusher", false, "Can the tool break when it runs out of energy", "config.canBreak");
        ...
        DrillTier.HOE = getDrillTierInfo("hoe", 1, 20000, 80, 80, EnumRarity.common, false, false, 2, 6.0F, 2.0F, 0);
    ...
    private DrillTier getDrillTierInfo(String tool, int tierNumber, int maxEnergy, int energyPerBlock, int rechargeRate, EnumRarity rarity, boolean canBreak, boolean hasModes, int miningLevel, float efficiency, float damage, int enchant){
        maxEnergy = config.getInt("Maximum Energy", tool + "_tier" + tierNumber, maxEnergy, 0, Integer.MAX_VALUE, "The max amount of energy that the tool can hold", "config.maxEnergy");
        energyPerBlock = config.getInt("Energy Per Block", tool + "_tier" + tierNumber, energyPerBlock, 0, Integer.MAX_VALUE, "The amount of energy that the tool uses to mine 1 block (uses twice as much energy for attacking)", "config.energyPerBlock");
        rechargeRate = config.getInt("Recharge Rate", tool + "_tier" + tierNumber, rechargeRate, 0, Integer.MAX_VALUE, "RF that the tool can recharge per tick", "config.rechargeRate");
        canBreak = config.getBoolean("Can Break", tool + "_tier" + tierNumber, canBreak, "Can the tool break when it runs out of energy", "config.canBreak");
        hasModes = config.getBoolean("Has Modes", tool + "_tier" + tierNumber, hasModes, "Can the tool change modes", "config.hasModes");
        miningLevel = config.getInt("Mining Level", tool + "_tier" + tierNumber, miningLevel, 0, Integer.MAX_VALUE, "Mining level of the tool (1 - can mine iron, 2 - can mine diamonds etc.)", "config.miningLevel");
        efficiency = config.getFloat("Efficiency", tool + "_tier" + tierNumber, efficiency, 0, Float.MAX_VALUE, "Mining speed of the tool", "config.efficiency");
        damage = config.getFloat("Damage", tool + "_tier" + tierNumber, damage, 0, Float.MAX_VALUE, "Damage that the tool does when attacking enemies", "config.damage");
        enchant = config.getInt("Enchantablility", tool + "_tier" + tierNumber, enchant, 0, Integer.MAX_VALUE, "Enchantablity of the tool (0 - not enchantable)", "config.enchant");
